...
if(isset($User->connect) AND $User->connect)
{
	...
	if(!empty(filter_input(INPUT_POST,'critere',FILTER_SANITIZE_STRING)) AND !empty(filter_input(INPUT_POST,'id',FILTER_SANITIZE_NUMBER_INT)))
	{
	...
	else if(!empty(filter_input(INPUT_GET,'critere',FILTER_SANITIZE_STRING)) AND !empty(filter_input(INPUT_GET,'id',FILTER_SANITIZE_NUMBER_INT)))
	{
	...
	if(!is_null($params))
	{
		$result = $Lord->deleteCriteres($critere,$params)->response->datas;
