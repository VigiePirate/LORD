...
class LORDRAT_API{
	...
	public function listRateries($order,$content,$start = 0,$quantity = 0)
	{
		$params = array(
			"order"		=> $order,
			"content"	=> $content,
	...
	public function addCriteres($critere,$nom)
	{
		//echo $critere." : ".$nom."\n";
		$params = array(
			"nom" => $nom
	...
	public function listCriteres($critere,$order,$content,$start = 0,$quantity = 0)
	{
		$params = array(
			"order"		=> $order,
			"content"	=> $content,
	...
	private function query($request,$params = array(),$method = "POST",$debug = FALSE)
	{
		...
		{
			$params_string .= $key.'='.htmlspecialchars($value,ENT_NOQUOTES).'&';
		}
		...
		$tempdecode = json_decode($response);
